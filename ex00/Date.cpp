...
static std::string extractChunk(
...
void Date::validate() {
    if (this->month > MONTH_MAX || this->month < MONTH_MIN) {
        throw std::runtime_error(std::string("Error: bad month => ").append(util::to_string(this->month)));
    }
    ...
    if (this->day < 1 || this->day > max_day) {
        throw std::runtime_error(std::string("Error: bad day => ").append(util::to_string(this->day)));
    }
}
...
int Date::parseIntChunk(const std::string& date_str) {
    ...
    iss >> value;
    if (iss.fail()) {
        throw std::runtime_error(std::string("Error: bad format => ").append(date_str));
    }
    return value;
}
...
Date::Date(const std::string& date) {
    ...
    if (year_str.size() > YEAR_DIGITS) {
        throw (std::runtime_error(std::string("Error: bad year => ").append(year_str)));
    }
...
int Date::toInt() const {
    return (this->year * YEAR_MULTIPLIER) + (this->month * MONTH_MULTIPLIER) + this->day;
}
